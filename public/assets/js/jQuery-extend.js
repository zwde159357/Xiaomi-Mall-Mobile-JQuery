(function() {
    ...
    $.extend({
        alert: function(msg) {
            ...
            $('<button>确定</button>').css(buttonCssConfig).on('click', function() { $alertEl.remove(); }).appendTo($buttonWrapper);
    ...
    $.extend({
        confirm: function(msg, callback) {
            ...
            $('<button>取消</button>').css(buttonCssConfig).on('click', function() { $confirmEl.remove(); }).appendTo($buttonWrapper);
    ...
    $.extend({
        myAjax: function(options) {
            var defaultOptions = {
				...
				headers: {   //强制让所有ajax请求在请求头的Authorization节点都携带token
            ...
            ajaxOptions.success = function(result, status, xhr) {
				//如果有新令牌在响应头中的authorization节点，则更新sessionStorage中的旧令牌，以刷新过期时间
